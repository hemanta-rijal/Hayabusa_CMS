...
@section('content')
    <div class="row">
        <form class="row" method="post" enctype='multipart/form-data' action="{{ route('help-banner.save') }}">
            @csrf
            @if(isset($banner->id))
...
@push('custom-js')
    ...
    <script>
        $(document).ready(function () {
            @if(isset($banner->id))
            singleImageEdit(".file-input-preview", "{!! $banner->image_link !!}", 2000);
            singleImageEdit(".background-image-preview", "{!! $banner->background_image_link !!}", 2000);
            @else
            singleImageCreate(".file-input-preview", 2000);
